(function () {
    ...
    function initMap(app) {
        ...
        var moduleVersion = app.dataset.moduleVersion || '1.0.10';
        var animationEnabled = app.dataset.animation !== '0';
        var reducedMotion = window.matchMedia &&
            window.matchMedia('(prefers-reduced-motion: reduce)').matches;

        var menuToggle = app.querySelector('[data-ampim-menu-toggle]');
        var menuClose = app.querySelector('[data-ampim-menu-close]');
        var layerMenu = app.querySelector('[data-ampim-layer-menu]');
        var layerGrid = app.querySelector('[data-ampim-layer-grid]');
        var currentLayerText = app.querySelector('[data-ampim-current-layer]');
        var previousButton = app.querySelector('[data-ampim-previous]');
        var playButton = app.querySelector('[data-ampim-play]');
        var nextButton = app.querySelector('[data-ampim-next]');
        var validity = app.querySelector('[data-ampim-validity]');
        var lead = app.querySelector('[data-ampim-lead]');
        var run = app.querySelector('[data-ampim-run]');
        var generated = app.querySelector('[data-ampim-generated]');
        var stale = app.querySelector('[data-ampim-stale]');
        var viewport = app.querySelector('[data-ampim-viewport]');
        var weatherCanvas = app.querySelector('[data-ampim-weather]');
        var vectorCanvas = app.querySelector('[data-ampim-vectors]');
        var labelsCanvas = app.querySelector('[data-ampim-labels]');
        ...
        var mapTitle = app.querySelector('[data-ampim-map-title]');
        var mapRun = app.querySelector('[data-ampim-map-run]');
        var mapDate = app.querySelector('[data-ampim-map-date]');
        var loading = app.querySelector('[data-ampim-loading]');
        var errorBox = app.querySelector('[data-ampim-error]');
        var slider = app.querySelector('[data-ampim-slider]');
        var timeline = app.querySelector('[data-ampim-timeline], .ampim-timeline');
        var periodSelector = app.querySelector('[data-ampim-period]');
        var dualRange = app.querySelector('[data-ampim-dual-range]');
        var periodStartSlider = app.querySelector('[data-ampim-period-start]');
        var periodEndSlider = app.querySelector('[data-ampim-period-end]');
        var periodTitle = app.querySelector('[data-ampim-period-title]');
        var periodSummary = app.querySelector('[data-ampim-period-summary]');
        var periodStartLabel = app.querySelector('[data-ampim-period-start-label]');
        var periodEndLabel = app.querySelector('[data-ampim-period-end-label]');
        var legend = app.querySelector('[data-ampim-legend]');
        var zoomIn = app.querySelector('[data-ampim-zoom-in]');
        ...
        var probeLabel = app.querySelector('[data-ampim-probe-label]');
        var toolButtons = app.querySelectorAll('[data-ampim-tool]');
        var toolHint = app.querySelector('[data-ampim-tool-hint]');
        var advancedTools = app.querySelector('[data-ampim-advanced-tools]');
        var captureButton = app.querySelector('[data-ampim-capture]');
        var pinButton = app.querySelector('[data-ampim-pin]');
        var diagramPopup = app.querySelector('[data-ampim-diagram-popup]');
        var diagramTitle = app.querySelector('[data-ampim-diagram-title]');
        var diagramBody = app.querySelector('[data-ampim-diagram-body]');
        var diagramStatus = app.querySelector('[data-ampim-diagram-status]');
        var diagramClose = app.querySelector('[data-ampim-diagram-close]');
        ...
        function probeCell(grid, x, y) {
            if (grid.values) {
                var value = grid.values[y * grid.width + x];
        ...
        function pinProbeAt(clientX, clientY) {
            ...
            pinnedElement.className = 'ampim-probe ampim-probe-pinned';
            ...
            close.type = 'button';
            close.className = 'ampim-probe-pin-close';
        ...
        function setToolMode(mode) {
            ...
            toolButtons.forEach(function (button) {
                var active = button.dataset.ampimTool === toolMode;
            ...
            if (toolMode === 'diagram') {
                setToolHint('Cliquez sur la carte pour afficher le diagramme AROME-PI du point choisi.');
            } else {
                setToolHint('');
                closeDiagram();
            }
        ...
        function renderDiagramChart(name, forecastRows, columnIndex, pointIndex) {
            ...
            svg.setAttribute('class', 'ampim-diagram-svg');
            svg.setAttribute('role', 'img');
            svg.setAttribute('aria-label', 'Diagramme AROME-PI pour ' + name);
            ...
            rains.forEach(function (value, index) {
                ...
                rect.setAttribute('class', 'ampim-diagram-rain');
            ...
            if (points.length > 1) {
                ...
                polyline.setAttribute('class', 'ampim-diagram-temp');
            ...
            [0, count - 1].forEach(function (index) {
                ...
                text.setAttribute('class', 'ampim-diagram-axis');
            ...
            caption.className = 'ampim-diagram-caption';
            caption.textContent = 'Température (ligne) et précipitations horaires (barres) — prochaines échéances AROME-PI.';
        ...
        function availableSteps() {
            ...
            return manifest.steps.filter(function (step) {
                var layer = manifest.layers[currentLayer];
                if (layer && layer.range_mode) {
                    var sourceKey = layer.source_key || currentLayer;
                    return step && step.probes && step.probes[sourceKey];
                }
                return step && step.files && step.files[currentLayer];
            });
        }

        function applyMapAspectRatio(bounds) {
            if (!bounds) { return; }
            ...
            if (![west, east, south, north].every(Number.isFinite) ||
                    east <= west || north <= south || south <= -85 || north >= 85) { return; }
            var ratio = ((east - west) * Math.PI / 180) /
                (mercator(north) - mercator(south));
            if (!Number.isFinite(ratio) || ratio <= 0) { return; }
            viewport.style.setProperty('--ampim-map-ratio', String(ratio));
        }

        // Legacy manifests already provide cumulative totals: no new model run is
        // necessary to compute end minus start. Never infer totals from hourly rain.
        function enablePrecipitationPeriod(payload) {
            var layer = payload.layers && payload.layers.pluie_cumul;
            if (!layer || layer.range_mode) { return; }
            var count = payload.steps.filter(function (step) {
                return step && step.probes && step.probes.pluie_cumul;
            }).length;
            if (count < 2) { return; }
            layer.label = 'Précipitations cumulées sur une période';
            layer.range_mode = 'difference';
            layer.source_key = 'pluie_cumul';
        }

        // Même principe que enablePrecipitationPeriod, pour les rafales : le
        // manifeste ne publie pas encore de couche "rafales_max" dédiée (ça
        // demanderait de redéployer le pipeline Python), donc on la construit
        // ici en clonant la définition déjà publiée de "rafales" — mêmes
        // couleurs, mêmes seuils — et en lui ajoutant les métadonnées de
        // période (source_key/range_mode) que le reste du code (déjà présent
        // pour la pluie) sait déjà interpréter.
        function enableGustPeriod(payload) {
            var source = payload.layers && payload.layers.rafales;
            var existing = payload.layers && payload.layers.rafales_max;
            if (!source || (existing && existing.range_mode)) { return; }
            var count = payload.steps.filter(function (step) {
                return step && step.probes && step.probes.rafales;
            }).length;
            if (count < 2) { return; }
            payload.layers.rafales_max = {
                label: 'Rafales maximales sur une période',
                unit: source.unit,
                group: source.group,
                decimals: source.decimals,
                transparent_below: source.transparent_below,
        ...
        function refreshLayerMenu() {
            ...
            layerGrid.querySelectorAll('[data-ampim-layer-key]').forEach(function (button) {
                var active = button.dataset.ampimLayerKey === currentLayer;
                button.classList.toggle('is-active', active);
                button.setAttribute('aria-pressed', active ? 'true' : 'false');
            });
        }
        ...
        function buildLayerMenu() {
            ...
            var columns = [0, 1, 2].map(function () {
                ...
                column.className = 'ampim-layer-column';
                layerGrid.appendChild(column);
                return column;
            });
            ...
            groupOrder.forEach(function (group) {
                ...
                section.className = 'ampim-layer-group';
                ...
                grouped[group].forEach(function (entry) {
                    ...
                    button.type = 'button';
                    button.className = 'ampim-layer-option' +
                        (essential ? '' : ' ampim-layer-option-secondary');
                    if (!essential) {
                        button.hidden = !showSecondaryLayers;
                    }
                    button.dataset.ampimLayerKey = entry.key;
                ...
                var columnIndex = group === 'Vent' || group === 'Nuages et humidité'
                    ? 2 : (group === 'Précipitations' || group === 'Instabilité' ? 1 : 0);
                columns[columnIndex].appendChild(section);
            });
            if (hasSecondary) {
                var moreButton = document.createElement('button');
                moreButton.type = 'button';
                moreButton.className = 'ampim-layer-more';
                moreButton.dataset.ampimLayerMore = '1';
        ...
        function buildLegend() {
            ...
            strip.className = 'ampim-legend-strip';
            layer.stops.forEach(function (stop) {
                var item = document.createElement('div');
                item.className = 'ampim-legend-stop';
        ...
        function updatePeriodControls() {
            ...
            if (steps.length < 2 || !periodStartSlider || !periodEndSlider) {
                return;
            }
            ...
            if (dualRange && dualRange.style && dualRange.style.setProperty) {
                ...
                dualRange.style.setProperty(
                    '--ampim-period-start', (periodStart / maximum * 100) + '%'
                );
                dualRange.style.setProperty(
                    '--ampim-period-end', (periodEnd / maximum * 100) + '%'
                );
            }
        }
        ...
        function adjustPeriodByKeyboard(event, startHandle) {
            var steps = availableSteps();
            if (steps.length < 2) { return; }
            var value = startHandle ? periodStart : periodEnd;
            var lower = startHandle ? 0 : periodStart + 1;
            var upper = startHandle ? periodEnd - 1 : steps.length - 1;
            if (event.key === 'ArrowRight' || event.key === 'ArrowUp') { value += 1; }
            ...
            stopAnimation();
            if (startHandle) { periodStart = clamp(value, lower, upper); }
            else { periodEnd = clamp(value, lower, upper); }
            updatePeriodControls();
        ...
        function renderPeriod() {
            ...
            var sourceKey = layer.source_key || currentLayer;
            mapTitle.textContent = layer.label + (layer.unit ? ' (' + layer.unit + ')' : '');
        ...
        function renderStep(index) {
            ...
            mapTitle.textContent = (layer ? layer.label : 'Carte AROME-PI') +
        ...
        if (periodStartSlider) {
            periodStartSlider.addEventListener('keydown', function (event) {
                adjustPeriodByKeyboard(event, true);
            });
        ...
        if (periodEndSlider) {
            periodEndSlider.addEventListener('keydown', function (event) {
                adjustPeriodByKeyboard(event, false);
            });
        ...
        toolButtons.forEach(function (button) {
            button.addEventListener('click', function () {
                setToolMode(button.dataset.ampimTool);
            });
        });
        if (captureButton) {
        ...
        viewport.addEventListener('pointerdown', function (event) {
            if (event.target.closest('button, .ampim-diagram-popup, .ampim-probe-pinned')) {
                return;
            }
            hideProbe();
            tapStart = {
        ...
        if (!baseUrl) {
            showError('Adresse des données AROME-PI non configurée.');
            return;
        }
        ...
        fetchJson(baseUrl + '/maps/index.json')
            .then(function (payload) {
                ...
                manifest = payload;
                applyMapAspectRatio(payload.bounds);
                enablePrecipitationPeriod(manifest);
                enableGustPeriod(manifest);
                buildLayerMenu();
                buildLegend();
                ...
                if (payload.run_time) {
                    run.textContent = 'Run du ' +
                        ...
                        ' • résolution 1,3 km';
                    mapRun.textContent = 'Run AROME-PI ' +
            ...
            .catch(function (error) {
                showError('Les cartes AROME-PI ne sont pas encore publiées : ' + error.message);
            });
    }

    whenReady(function () {
        document.querySelectorAll('[data-ampim-app]').forEach(initMap);
    });
}());
