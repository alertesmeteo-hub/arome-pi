(function () {
    ...
    function temperatureClass(value) {
        ...
        if (value >= 35) { return 'ampi-temp-extreme'; }
        if (value >= 30) { return 'ampi-temp-hot'; }
        if (value >= 22) { return 'ampi-temp-warm'; }
        if (value >= 12) { return 'ampi-temp-mild'; }
        if (value >= 4) { return 'ampi-temp-cool'; }
        return 'ampi-temp-cold';
    }

    function gustClass(value) {
        if (!finite(value)) { return ''; }
        if (value >= 80) { return 'ampi-gust-danger'; }
        if (value >= 60) { return 'ampi-gust-strong'; }
        if (value >= 40) { return 'ampi-gust-moderate'; }
        return '';
    }

    function windForceClass(value) {
        if (!finite(value)) { return 'ampi-wind-calm'; }
        if (value >= 80) { return 'ampi-wind-violent'; }
        if (value >= 60) { return 'ampi-wind-strong'; }
        if (value >= 40) { return 'ampi-wind-fresh'; }
        if (value >= 20) { return 'ampi-wind-moderate'; }
        return 'ampi-wind-light';
    }

    function rainClass(value) {
        if (!finite(value) || value < 0.1) { return ''; }
        if (value >= 15) { return 'ampi-rain ampi-rain-intense'; }
        if (value >= 5) { return 'ampi-rain ampi-rain-strong'; }
        if (value >= 2) { return 'ampi-rain ampi-rain-moderate'; }
        if (value >= 0.5) { return 'ampi-rain ampi-rain-light'; }
        return 'ampi-rain ampi-rain-trace';
    }
    ...
    function chartDayBands(svg, count, margin, innerWidth, innerHeight, meta) {
        ...
        while (start < count) {
            ...
            svg.appendChild(svgElement('rect', {
                ...
                height: innerHeight,
                class: 'ampi-chart-day-band ampi-dayband-' + token
            }));
            ...
            svg.appendChild(svgElement('rect', {
                ...
                width: Math.max(0, endX - startX),
                height: 46,
                class: 'ampi-chart-day-strip ampi-daystrip-' + token
            }));
            ...
            if (start > 0) {
                svg.appendChild(svgElement('line', {
                    ...
                    class: 'ampi-chart-day-separator ampi-dayseparator-' + token
                }));
            }
            ...
            if (endX - startX > 72) {
                ...
                svg.appendChild(svgElement('text', {
                    ...
                    y: margin.top + 31,
                    'text-anchor': 'middle',
                    class: 'ampi-chart-day-label ampi-daylabel-' + token
                }, dayLabel));
            }
            start = end + 1;
        }
    ...
    function appendChartMetrics(container, metrics) {
        ...
        block.className = 'ampi-chart-metrics';
        valid.forEach(function (metric) {
            var item = document.createElement('span');
            item.className = 'ampi-chart-metric';
            item.appendChild(textSpan(metric.label, 'ampi-chart-metric-label'));
            item.appendChild(textSpan(metric.value, 'ampi-chart-metric-value'));
    ...
    function appendChartViewport(container, svg) {
        ...
        viewport.className = 'ampi-chart-viewport';
        var tooltip = document.createElement('div');
        tooltip.className = 'ampi-chart-tooltip';
        tooltip.hidden = true;
        ...
        function tooltipTarget(node) {
            ...
            return node.closest('[data-ampi-tooltip]');
        }
        ...
        viewport.addEventListener('pointerover', function (event) {
            ...
            if (!target) { return; }
            tooltip.textContent = target.getAttribute('data-ampi-tooltip') || '';
    ...
    function drawExtremaLabels(svg, points, values, formatter, className) {
        ...
        indexes.forEach(function (index) {
            ...
            svg.appendChild(svgElement('text', {
                ...
                y: y,
                'text-anchor': 'middle',
                class: 'ampi-chart-value-label ' + className
    ...
    function renderLineChart(container, labels, series, options) {
        ...
        if (!scale) {
            container.appendChild(textSpan('Données indisponibles pour ce diagramme.', 'ampi-chart-empty'));
            return;
        }
        ...
        svg.classList.add('ampi-chart-svg');
        ...
        for (var g = 0; g <= tickValue; g += 1) {
            ...
            svg.appendChild(svgElement('line', {
                ...
                y1: gy,
                y2: gy,
                class: 'ampi-chart-grid'
            }));
            svg.appendChild(svgElement('text', {
                x: margin.left - 9,
                y: gy + 4,
                'text-anchor': 'end',
                class: 'ampi-chart-axis'
        ...
        chartTickIndexes(n, 8).forEach(function (index) {
            ...
            svg.appendChild(svgElement('line', {
                ...
                y1: margin.top,
                y2: margin.top + innerHeight,
                class: 'ampi-chart-grid ampi-chart-grid-vertical'
            }));
            svg.appendChild(svgElement('text', {
                x: x,
                y: height - 18,
                'text-anchor': index === 0 ? 'start' : (index === n - 1 ? 'end' : 'middle'),
                class: 'ampi-chart-axis ampi-chart-xaxis'
            }, axisLabel));
        });
        ...
        series.forEach(function (item, seriesIndex) {
            ...
            if (series.length === 1 && options.fillArea) {
                segments.forEach(function (segment) {
                    ...
                    svg.appendChild(svgElement('path', {
                        d: d,
                        class: 'ampi-chart-area ' + item.className
                    }));
                });
            }
            ...
            segments.forEach(function (segment) {
                svg.appendChild(svgElement('path', {
                    ...
                    class: 'ampi-chart-line ' + item.className,
                    fill: 'none'
                }));
            });
            ...
            points.forEach(function (point, index) {
                ...
                var attrs = {
                    ...
                    class: (options.showPoints === false ? 'ampi-chart-hitpoint ' : 'ampi-chart-point ') + item.className,
                    tabindex: 0,
                    'aria-label': tooltipLabel,
                    'data-ampi-tooltip': tooltipLabel
                };
        ...
        if (options.bandBetween && seriesPoints.length >= 2) {
            ...
            if (upperPoints.length > 1 && upperPoints.length === lowerPoints.length) {
                ...
                var envelope = svgElement('path', {
                    d: area,
                    class: 'ampi-chart-wind-envelope'
                });
                var firstLine = svg.querySelector('.ampi-chart-line');
        ...
        if (series.length > 1) {
            var legend = svgElement('g', { class: 'ampi-chart-legend' });
            series.forEach(function (item, index) {
                ...
                legend.appendChild(svgElement('text', {
                    x: x + 36,
                    y: 21,
                    class: 'ampi-chart-axis ampi-chart-legend-text'
                }, item.label));
            });
    ...
    function rainVisualClass(value) {
        if (!finite(value) || value < 0.1) { return 'ampi-chart-rain-zero'; }
        if (value >= 15) { return 'ampi-chart-rain-intense'; }
        if (value >= 5) { return 'ampi-chart-rain-strong'; }
        if (value >= 2) { return 'ampi-chart-rain-moderate'; }
        if (value >= 0.5) { return 'ampi-chart-rain-light'; }
        return 'ampi-chart-rain-trace';
    }
    ...
    function renderRainChart(container, labels, rainValues, cumulativeValues, options) {
        ...
        for (var cg = 0; cg <= cumTickCount; cg += 1) {
            ...
            svg.appendChild(svgElement('text', {
                ...
                y: cy + 5,
                'text-anchor': 'start',
                class: 'ampi-chart-axis ampi-chart-axis-cumulative'
        ...
        rainValues.forEach(function (value, index) {
            ...
            var bar = svgElement('rect', {
                ...
                class: 'ampi-chart-rain-bar ' + rainVisualClass(value),
                tabindex: 0,
                'aria-label': rainTooltip,
                'data-ampi-tooltip': rainTooltip
            });
            svg.appendChild(bar);
        });
        ...
        if (cleanPoints.length) {
            svg.appendChild(svgElement('path', {
                ...
                class: 'ampi-chart-line ampi-series-rain-cumulative',
                fill: 'none'
            }));
            cumulativePoints.forEach(function (point, index) {
                ...
                var hitPoint = svgElement('circle', {
                    ...
                    cy: point.y,
                    r: 11,
                    class: 'ampi-chart-hitpoint ampi-series-rain-cumulative',
                    tabindex: 0,
                    'aria-label': cumulativeTooltip,
                    'data-ampi-tooltip': cumulativeTooltip,
                    fill: 'transparent',
                    stroke: 'none',
                    opacity: '0'
                });
        ...
        legend.appendChild(svgElement('rect', {
            ...
            height: 11,
            rx: 4,
            class: 'ampi-chart-rain-bar ampi-chart-rain-light'
        }));
        legend.appendChild(svgElement('text', {
            x: margin.left + 28,
            y: 21,
    ...
    function initApp(app) {
        ...
        var hours = Math.max(1, Math.min(6, parseInt(app.dataset.hours || '6', 10)));
        var timezone = app.dataset.timezone || 'Europe/Paris';
        var titlePrefix = app.dataset.titlePrefix || 'Prévisions AROME-PI';

        var input = app.querySelector('.ampi-city-input');
        var locateButton = app.querySelector('[data-ampi-locate]');
        var results = app.querySelector('.ampi-search-results');
        var status = app.querySelector('.ampi-search-status');
        var generalBody = app.querySelector('[data-ampi-body-general]');
        var stormBody = app.querySelector('[data-ampi-body-storms]');
        var stormSummary = app.querySelector('[data-ampi-storm-summary]');
        var snowBody = app.querySelector('[data-ampi-body-snow]');
        var snowSummary = app.querySelector('[data-ampi-snow-summary]');
        var tabs = Array.prototype.slice.call(app.querySelectorAll('[data-ampi-tab]'));
        var panels = Array.prototype.slice.call(app.querySelectorAll('[data-ampi-panel]'));
        var title = app.querySelector('[data-ampi-title]');
        var altitudeLine = app.querySelector('[data-ampi-altitude]');
        ...
        var chartWind = app.querySelector('[data-ampi-chart-wind]');
        var chartTitleTemperature = app.querySelector('[data-ampi-chart-title-temperature]');
        var chartTitlePressure = app.querySelector('[data-ampi-chart-title-pressure]');
        var chartTitleRain = app.querySelector('[data-ampi-chart-title-rain]');
        var chartTitleWind = app.querySelector('[data-ampi-chart-title-wind]');
        var rainTotal = app.querySelector('[data-ampi-rain-total]');
        ...
        function makeDayCell(date, rowSpan) {
            var cell = createCell('th', 'Date', 'ampi-day-cell ampi-day-' + weekdayToken(date));
            cell.scope = 'rowgroup';
            cell.rowSpan = rowSpan;
            cell.appendChild(textSpan(weekdayFormat.format(date), 'ampi-day-weekday'));
            cell.appendChild(textSpan(shortDateFormat.format(date), 'ampi-day-date'));
            return cell;
        }
        ...
        function setStatus(message, error) {
            ...
            status.classList.toggle('ampi-search-error', Boolean(error));
        }

        function setActiveView(view) {
            tabs.forEach(function (button) {
                var active = button.dataset.ampiTab === view;
        ...
        function putMessage(body, message, error, colspan) {
            ...
            var cell = createCell('td', '', error ? 'ampi-loading ampi-load-error' : 'ampi-loading');
        ...
        function showTableMessage(message, error) {
            ...
            if (stormSummary) {
                ...
                stormSummary.classList.toggle('ampi-storm-summary-warning', Boolean(error));
            }
            if (snowSummary) {
                snowSummary.textContent = message;
                snowSummary.classList.toggle('ampi-snow-summary-warning', Boolean(error));
            }
        }
            ...
            return fetchJson(baseUrl + '/index.json', { cache: 'no-cache' })
                .then(function (payload) {
                    if (!payload || payload.status !== 'ok' || !payload.departments) {
                        throw new Error('Index national AROME-PI invalide');
                    }
                    indexData = payload;
        ...
        function loadDepartment(code) {
            ...
            if (!indexData || !indexData.departments[normalized]) {
                return Promise.reject(new Error('Département hors couverture AROME-PI'));
            }
        ...
        function renderForecast(departmentData, commune) {
            ...
            function appendHazard(row, label, code) {
                ...
                var cell = createCell('td', label, numeric === null ? '' : 'ampi-hazard ampi-hazard-' + numeric);
            ...
            if (!forecasts.length) {
                showTableMessage('Aucune échéance AROME-PI disponible pour cette commune.', true);
                return;
            }
            ...
            forecasts.forEach(function (step, currentIndex) {
                ...
                if (dayKey !== previousGeneralDay) {
                    row.classList.add('ampi-new-day');
                    row.appendChild(makeDayCell(date, dayCounts[dayKey]));
                    previousGeneralDay = dayKey;
                }

                var hourCell = createCell('td', 'Heure', 'ampi-hour ampi-hour-' + weekdayToken(date));
                ...
                var conditionCell = createCell('td', 'Temps', 'ampi-condition');
                conditionCell.appendChild(textSpan(condition.icon, 'ampi-icon'));
                conditionCell.appendChild(textSpan(condition.label));
                row.appendChild(conditionCell);

                var temperatureCell = createCell('td', 'Température', 'ampi-temperature ' + temperatureClass(temp));
                ...
                var windCell = createCell('td', 'Vent', 'ampi-wind-cell ' + windForceClass(windDisplay));
                ...
                if (dayKey !== previousStormDay) {
                    stormRow.classList.add('ampi-new-day');
                    stormRow.appendChild(makeDayCell(date, dayCounts[dayKey]));
                    previousStormDay = dayKey;
                }
                var stormHour = createCell('td', 'Heure', 'ampi-hour ampi-hour-' + weekdayToken(date));
                ...
                var thunderCell = createCell('td', 'Risque orage', thunder ? 'ampi-thunder-risk ampi-thunder-' + Number(thunderCode) : '');
                if (thunder) {
                    thunderCell.appendChild(textSpan(thunder.icon + ' ', 'ampi-risk-icon'));
                    thunderCell.appendChild(textSpan(thunder.label, 'ampi-risk-label'));
                } else {
                ...
                var capeCell = createCell('td', 'CAPE', finite(cape) && cape >= 1500 ? 'ampi-cape-high' : (finite(cape) && cape >= 500 ? 'ampi-cape-mid' : ''));
                capeCell.textContent = finite(cape) && Number(cape) >= 25 ? formatNumber(cape, 0) + ' J/kg' : '—';
                capeCell.title = 'MUCAPE instantanée directement produite par AROME-PI ; les valeurs quasi nulles ne sont pas affichées.';
                stormRow.appendChild(capeCell);
                appendNumber(stormRow, 'LCL', value(values, 'lcl_m'), 0, ' m');

                var lightning = value(values, 'lightning_score');
                appendNumber(stormRow, 'Foudre', lightning, 0, '/100', finite(lightning) && lightning >= 60 ? 'ampi-lightning-high' : '');
                ...
                var stormTypeCell = createCell('td', 'Type d’orage', 'ampi-storm-type');
                ...
                var detailsCell = createCell('td', 'Détails', 'ampi-details-cell');
                var detailsButton = document.createElement('button');
                detailsButton.type = 'button';
                detailsButton.className = 'ampi-details-button';
                detailsButton.textContent = 'Afficher';
                detailsButton.setAttribute('aria-expanded', 'false');
                detailsCell.appendChild(detailsButton);

                var advanced = document.createElement('div');
                advanced.className = 'ampi-advanced-list ampi-details-inline';
                ...
                [
                    ...
                    box.className = 'ampi-advanced-item';
                    box.appendChild(textSpan(item[0], 'ampi-advanced-label'));
                    box.appendChild(textSpan(finite(item[1]) ? formatNumber(item[1], item[2]) + item[3] : '—', 'ampi-advanced-value'));
                ...
                if (dayKey !== previousSnowDay) {
                    snowRow.classList.add('ampi-new-day');
                    snowRow.appendChild(makeDayCell(date, dayCounts[dayKey]));
                    previousSnowDay = dayKey;
                }
                var snowHour = createCell('td', 'Heure', 'ampi-hour ampi-hour-' + weekdayToken(date));
                ...
                var snowRiskCell = createCell('td', 'Risque neige', snowRisk ? 'ampi-snow-risk ampi-snow-' + Number(snowRiskCode) : '');
                if (snowRisk) {
                    snowRiskCell.appendChild(textSpan(snowRisk.icon + ' ', 'ampi-risk-icon'));
                    snowRiskCell.appendChild(textSpan(snowRisk.label, 'ampi-risk-label'));
                ...
                var stickCell = createCell('td', 'Tenue', finite(stickCode) ? 'ampi-stick ampi-stick-' + Number(stickCode) : '');
                ...
                var snowWindCell = createCell('td', 'Vent moyen / rafales', 'ampi-snow-wind ' + windForceClass(gustDisplay));
                ...
                var snowDetailsCell = createCell('td', 'Détails', 'ampi-details-cell ampi-snow-details-cell');
                var snowDetailsButton = document.createElement('button');
                snowDetailsButton.type = 'button';
                snowDetailsButton.className = 'ampi-details-button';
                ...
                snowAdvanced.className = 'ampi-snow-profile ampi-details-inline';
                snowAdvanced.hidden = true;

                function appendProfileSection(titleText, items) {
                    var section = document.createElement('section');
                    section.className = 'ampi-profile-section';
                    section.appendChild(textSpan(titleText, 'ampi-profile-title'));
                    var list = document.createElement('div');
                    list.className = 'ampi-profile-list';
                    items.forEach(function (item) {
                        var profileItem = document.createElement('div');
                        profileItem.className = 'ampi-profile-item';
                        profileItem.appendChild(textSpan(item[0], 'ampi-profile-label'));
                        profileItem.appendChild(textSpan(finite(item[1]) ? formatNumber(item[1], item[2]) + item[3] : '—', 'ampi-profile-value'));
                ...
                appendProfileSection('Sorties et diagnostics AROME-PI', [
            ...
            if (stormSummary) {
                stormSummary.classList.toggle('ampi-storm-summary-warning', !hasStormColumns);
                if (!hasStormColumns) {
                    ...
                    stormSummary.textContent = 'Risque orageux maximal sur les ' + forecasts.length + ' prochaines échéances : ' + maxRisk.icon + ' ' + maxRisk.label + ' • MUCAPE et réflectivité AROME-PI 0,01°.';
                }
            }
            if (snowSummary) {
                snowSummary.classList.toggle('ampi-snow-summary-warning', !hasSnowColumns);
                if (!hasSnowColumns) {
                    ...
                    snowSummary.textContent = 'Risque de neige maximal sur les ' + forecasts.length + ' prochaines échéances : ' + maxSnow.icon + ' ' + maxSnow.label + ' • cumul neige AROME-PI 0,01°.';
                }
            }
            ...
            renderLineChart(chartTemperature, chartLabels, [{ label: 'Température', values: chartTemps, className: 'ampi-series-temperature' }], {
                decimals: 0,
                unit: '°C',
                fillArea: false,
            ...
            renderLineChart(chartPressure, chartLabels, [{ label: 'Pression', values: chartPressures, className: 'ampi-series-pressure' }], {
                decimals: 0,
                unit: 'hPa',
                fillArea: false,
                metricMode: 'pressure',
            ...
            renderLineChart(chartWind, chartLabels, [
                { label: 'Vent moyen', values: chartWindValues, className: 'ampi-series-wind' },
                { label: 'Rafales', values: chartGustValues, className: 'ampi-series-gust' }
            ], {
                decimals: 0,
                unit: 'km/h',
                forceZero: true,
        ...
        function selectCommune(candidate) {
            ...
            loadDepartment(candidate.codeDepartement)
                .then(function (departmentData) {
                    ...
                    if (!commune) {
                        throw new Error('Commune absente du catalogue AROME-PI');
                    }
        ...
        function displaySearchResults(candidates) {
            ...
            candidates.forEach(function (candidate, position) {
                var button = document.createElement('button');
                button.type = 'button';
                button.className = 'ampi-search-result';
                button.id = input.id + '-option-' + position;
                button.setAttribute('role', 'option');
                button.setAttribute('aria-selected', 'false');

                var name = textSpan(candidate.nom, 'ampi-result-name');
        ...
        if (window.AMPI_AUTOHEAL && window.AMPI_AUTOHEAL.url) {
            fetch(window.AMPI_AUTOHEAL.url, { method: 'POST', cache: 'no-store' }).catch(function () {});
        }
    ...
    whenReady(function () {
        document.querySelectorAll('[data-ampi-app]').forEach(initApp);
    });
}());
